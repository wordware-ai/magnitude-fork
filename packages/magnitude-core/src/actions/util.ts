...
class SchemaAdder {
  ...
  private _parseObject(jsonSchema: JsonSchema): FieldType {
    if (jsonSchema.type !== "object") {
      throw new Error(
        `_parseObject called with non-object type: ${jsonSchema.type}`
      );
    }
    ...
    if (
      ...
      !requiredFields.every((rf) => typeof rf === "string")
    ) {
      throw new Error(
        `'required' property in object '${name}' must be an array of strings.`
      );
    }
    ...
    if (properties && typeof properties === "object") {
      for (const [fieldName, fieldSchemaUntyped] of Object.entries(
        ...
        if (
          typeof fieldSchemaUntyped !== "object" ||
          fieldSchemaUntyped === null
        ) {
          console.warn(
            `Property schema for '${fieldName}' in class '${name}' is not a valid object/schema. Skipping.`
          );
          continue;
        }
  ...
  private _parseString(jsonSchema: JsonSchema): FieldType {
    if (jsonSchema.type !== "string") {
      throw new Error(
        `_parseString called with non-string type: ${jsonSchema.type}`
      );
    }
    ...
    if (enumValues) {
      if (!Array.isArray(enumValues)) {
        throw new Error(
          `'enum' property for string type '${title || "anonymous"}' must be an array.`
        );
      }
      ...
      if (title === undefined || typeof title !== "string") {
        if (stringEnumValues.length === 0) {
          console.warn(
            `Anonymous enum (string type with 'enum' but no 'title') has no values. Defaulting to plain string type.`
          );
          return this.tb.string();
        }
        return this.tb.union(
          stringEnumValues.map((value) => this.tb.literalString(value))
        );
      }
      ...
      if (stringEnumValues.length === 0) {
        console.warn(
          `Enum '${title}' has no values. An empty enum was created.`
        );
      }
  ...
  private _parseArray(jsonSchema: JsonSchema): FieldType {
    if (jsonSchema.type !== "array") {
      throw new Error(
        `_parseArray called with non-array type: ${jsonSchema.type}`
      );
    }
    ...
    if (itemsSchema === undefined) {
      throw new Error(
        `Array field '${jsonSchema.title || "untitled array"}' is missing 'items' definition.`
      );
    }
    ...
    if (
      ...
      throw new Error(
        `'items' property for array '${jsonSchema.title || "untitled array"}' must be a single schema object. Tuple/array types for 'items' are not supported by this converter (matching Python script's direct parsing of items).`
      );
    }
  ...
  private _loadRef(ref: string): FieldType {
    ...
    if (pathParts.length !== 2 || !pathParts[0] || !pathParts[1]) {
      throw new Error(
        `Unsupported $ref format: '${ref}'. Expected format like '#/collectionKey/definitionKey' matching Python logic.`
      );
    }
    ...
    if (typeof collection !== "object" || collection === null) {
      throw new Error(
        `Reference collection '${collectionKey}' for $ref '${ref}' not found or not an object in the root schema.`
      );
    }

    if (!collection.hasOwnProperty(definitionKey)) {
      throw new Error(
        `Reference item '${definitionKey}' for $ref '${ref}' not found in collection '${collectionKey}'.`
      );
    }
  ...
  parse(jsonSchema: JsonSchema): FieldType {
    ...
    if (jsonSchema.anyOf) {
      ...
      if (jsonSchema.anyOf.length === 0) {
        console.warn(
          "'anyOf' is an empty array. Defaulting to string (Python script has no specific handling)."
        );
        return this.tb.string();
      }
      return this.tb.union(
        jsonSchema.anyOf.map((subSchema) => this.parse(subSchema as JsonSchema))
      );
    }
    ...
    if (
      ...
      (additionalProperties as JsonSchema).anyOf
    ) {
      const apSchema = additionalProperties as JsonSchema;
      const anyOfInAp = apSchema.anyOf;
      if (!Array.isArray(anyOfInAp)) {
        throw new Error(
          "'anyOf' in 'additionalProperties' must be an array of schema objects."
        );
      }
      if (!anyOfInAp.every((s) => typeof s === "object" && s !== null)) {
        throw new Error(
          "'anyOf' in 'additionalProperties' must contain valid schema objects."
        );
      }
      if (anyOfInAp.length === 0) {
        console.warn(
          "'anyOf' in 'additionalProperties' is empty. Defaulting map value to string."
        );
        return this.tb.map(this.tb.string(), this.tb.string());
      }
      const valueType = this.tb.union(
        anyOfInAp.map((subSchema) => this.parse(subSchema as JsonSchema))
      );
    ...
    if (type === undefined) {
      console.warn(
        "Type field is missing in JSON schema, defaulting to string (Python script behavior)."
      );
      return this.tb.string();
    }
    ...
    if (typeof type !== "string") {
      throw new Error(
        `Unsupported 'type' format: Expected a string, got ${typeof type} (${JSON.stringify(type)}). Python script would raise ValueError.`
      );
    }
...
function convertJsonSchemaToBaml(
  tb: TypeBuilder,
