...
description: "Reference for the low-level AI methods `ai.click()`, `ai.type()`, and `ai.exec()` for direct browser interaction control."
---

While `ai.step()` is the primary method for defining actions using natural language, the `ai` object also provides lower-level methods for more direct control over specific browser interactions. These methods bypass the AI's interpretation for the specific action described.

They can be useful when:
- The AI's interpretation of a natural language step is ambiguous.
- You need precise control over a specific interaction.
- You are integrating with existing automation logic that generates specific action intents.
...
Executes a predefined `ActionIntent` object. This is the lowest-level method, typically used for advanced scenarios or internal integrations.
